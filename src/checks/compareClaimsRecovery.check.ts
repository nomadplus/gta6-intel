/**
 * Regression check for Phase 5 PR 7's recovery mutation
 * (src/db/mutations/compareClaimsRecovery.ts) -- exercises the REAL
 * reclaimStaleInFlightCompareClaimsJob() against a real local Postgres
 ...
 * detectDuplicatesRecovery.check.ts, identity narrowed from one
 * (extractionAiResultId, extractionCandidateIndex) pair to one
 * comparisonClaimId (migration 0021), proving the same race-safe
 ...
 *     focus claim despite the partial unique index (migration 0021)
 *   - two concurrent recovery attempts on a focus claim with NO existing
 *     in-flight job cannot both create a new in-flight job
 *   - no focus claim with any in-flight job at all: reclaim is a safe no-op
 *   - ZERO admin_audit_log rows are written by any recovery outcome
 *     (fresh_in_flight, reclaimed, or none) -- matching the PR3/PR4/PR6
 *     convention that trigger/recovery actions are never themselves
 *     audited
 *
 * Unlike detectDuplicatesRecovery.check.ts, there is deliberately NO
 * "reviewed candidate blocks recovery" case here: compare_claims has no
 * per-focus-claim reviewed/unreviewed concept analogous to a single
 * extract_claims candidate's review (the underlying assessments are
 * reviewed individually, and PR7 provides no re-analysis control from a
 * succeeded state regardless -- see compareClaimsRecovery.ts's own
 * header comment).
 *
 * server-only-guarded, so this must run with --conditions=react-server.
 *
 * Run with: npx tsx --conditions=react-server src/checks/compareClaimsRecovery.check.ts
...
import { COMPARE_CLAIMS_RECOVERY_STALE_THRESHOLD_MS } from "../lib/ai/compareClaimsRecoveryLifecycle";
...
async function main() {
  ...
  if (!process.env.LOCAL_FAKE_ADMIN_AUTH_USER_ID) {
    throw new Error("LOCAL_FAKE_ADMIN_AUTH_USER_ID is not set -- required so reclaimStaleInFlightCompareClaimsJob's internal requireAdmin() call can resolve a session. See README.md.");
  }
  ...
  async function createTestClaim(statement: string): Promise<number> {
    const [row] = await db.insert(claims).values({ projectId: SEEDED_PROJECT_ID, slug: `pr7-recovery-claim-${randomUUID()}`, statement, informationType: "report" }).returning();
    return row.id;
  }

  async function insertInFlightCompareClaimsJob(comparisonClaimId: number, status: "pending" | "running", ageMs: number): Promise<number> {
  ...
  try {
    console.log("=== compare_claims recovery mutation (Phase 5 PR 7) -- DB only, no AI calls ===\n");
    ...
    // --- fresh pending job: NOT recovery-eligible, no DB change -----------
    {
      ...
      const jobId = await insertInFlightCompareClaimsJob(claimId, "pending", 1000);
      const before = await loadJob(jobId);

      const outcome = await reclaimStaleInFlightCompareClaimsJob(claimId);
    ...
    // --- fresh running job: NOT recovery-eligible, no DB change -----------
    {
      ...
      const jobId = await insertInFlightCompareClaimsJob(claimId, "running", 1000);
      const before = await loadJob(jobId);
    ...
    // --- stale pending job: reclaimed, terminal timestamp set, row preserved ---
    {
      ...
      const jobId = await insertInFlightCompareClaimsJob(claimId, "pending", COMPARE_CLAIMS_RECOVERY_STALE_THRESHOLD_MS + 60_000);
    ...
    // --- stale running job: reclaimed the same way ------------------------
    {
      ...
      const jobId = await insertInFlightCompareClaimsJob(claimId, "running", COMPARE_CLAIMS_RECOVERY_STALE_THRESHOLD_MS + 60_000);
    ...
    // partial unique index (migration 0021) -----------------------------
    {
      const claimId = await createTestClaim("Post-reclaim replacement fixture claim.");
      await insertInFlightCompareClaimsJob(claimId, "pending", COMPARE_CLAIMS_RECOVERY_STALE_THRESHOLD_MS + 60_000);

      const reclaimOutcome = await reclaimStaleInFlightCompareClaimsJob(claimId);
      assert(reclaimOutcome.outcome === "reclaimed", "setup: the stale job is reclaimed before the replacement attempt");

      const replacement = await createPendingAiJob({ operation: "compare_claims", provider: "fake", model: "test-model", comparisonClaimId: claimId });
      assert(replacement.ok === true, "a replacement pending job CAN be created for the same focus claim after the stale job was reclaimed");
    }
    ...
    // --- no in-flight job at all: reclaim is a safe no-op -----------------
    {
      ...
      const outcome = await reclaimStaleInFlightCompareClaimsJob(claimId);
      assert(outcome.outcome === "none", "a focus claim with NO in-flight job returns {outcome: 'none'}");
    }
    ...
    // --- concurrent recovery cannot create two in-flight jobs -------------
    {
      ...
      const [first, second] = await Promise.all([
        createPendingAiJob({ operation: "compare_claims", provider: "fake", model: "test-model", comparisonClaimId: claimId }),
        createPendingAiJob({ operation: "compare_claims", provider: "fake", model: "test-model", comparisonClaimId: claimId }),
      ]);
    ...
    // detectDuplicatesRecovery.check.ts/claimProposalReview.check.ts.
    await pool.end();
  }

  console.log(failures === 0 ? "\nAll compare_claims recovery mutation checks passed." : `\n${failures} check(s) FAILED.`);
